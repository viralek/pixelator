...
palette_rgb = [tuple(int(color[i:i+2], 16) for i in (1, 3, 5)) for color in PALETTE_COLORS]
...
def apply_palette(img, palette):
    ...
    distances = np.sqrt(((reshaped_img[:, None, :] - palette) ** 2).sum(axis=2))
    closest_colors = np.argmin(distances, axis=1)
    return np.array(palette)[closest_colors].reshape(img.shape).astype(np.uint8)
...
while True:
    ...
    if USE_PALETTE:
        pixelated_frame = apply_palette(pixelated_frame, palette_rgb)
