...
while True:
    ...
    if countdown_active:
        countdown_elapsed = int(time.time() - countdown_start)
        countdown_number = 3 - countdown_elapsed
        if countdown_number <= 0:
            filename = datetime.now().strftime('imgexp/capture_%Y%m%d_%H%M%S.png')
            cv2.imwrite(filename, final_frame)
            countdown_active = False
        else:
            cv2.putText(final_frame, str(countdown_number), (FRAME_WIDTH // 2 - 40, FRAME_HEIGHT // 2 + 40), cv2.FONT_HERSHEY_SIMPLEX, 4, (255,255,255), 6)
    ...
    final_frame_disp = np.vstack([final_frame, info_bar])
    cv2.imshow('8-bit Pixelated People', final_frame_disp)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('a') and not countdown_active:
